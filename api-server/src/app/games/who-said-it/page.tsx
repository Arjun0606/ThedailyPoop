...
export default function WhoSaidItPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const res = await fetch(`/api/games/who-said-it/today?userId=${session.user.id}`);
  ...
  const handleAnswer = useCallback(
    async (answer: string) => {
      ...
      if (currentIndex < game.rounds.length - 1) {
        ...
        const res = await fetch("/api/games/who-said-it/submit", {
          method: "POST",
          headers: {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-black px-4">
        <div className="w-full max-w-sm text-center">
          ...
          <h1 className="mt-4 text-2xl font-black text-white">Who Said It?</h1>
          <p className="mt-2 text-sm text-[var(--text-secondary)]">
            Match the unhinged quote to the public figure. Pro subscribers only.
          </p>
        ...
        <div className="mx-auto max-w-lg">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              {pct >= 80 ? "You know your politicians!" : pct >= 50 ? "Not bad, but some fooled you." : "These quotes are wild, huh?"}
            </p>
          </div>
          ...
          <div className="mt-8 space-y-3">
            {results.map((r, i) => (
              <div key={i} className={`glass-card p-4 ${r.correct ? "border-green-500/30 bg-green-500/5" : "border-red-500/30 bg-red-500/5"}`}>
                <p className="text-sm italic text-zinc-300">&ldquo;{r.quote}&rdquo;</p>
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                  <span className={r.correct ? "text-green-400" : "text-red-400"}>{r.correct ? "Correct" : "Wrong"}</span>
                  <span className="text-[var(--text-tertiary)]">&middot;</span>
                  <span className="text-[var(--text-secondary)]">Said by: <strong className="text-white">{r.correctAnswer}</strong></span>
                  {!r.correct && (
                    <>
                      <span className="text-[var(--text-tertiary)]">&middot;</span>
                      <span className="text-red-400/70">You said: {r.userAnswer}</span>
                    </>
                  )}
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-black px-4 py-8">
      <div className="mx-auto max-w-lg">
        ...
        <div className="text-center">
          <Image src="/logo.png" alt="" width={48} height={48} className="mx-auto drop-shadow-lg" />
          <h1 className="mt-2 text-xl font-black text-white">Who Said It?</h1>
        ...
        <div className="mt-8 glass-card p-6">
          <p className="text-lg font-bold leading-snug text-white italic">&ldquo;{round.quote}&rdquo;</p>
          <p className="mt-3 text-xs text-[var(--text-tertiary)]">Who said this?</p>
        </div>
